...
export const getBorrowRecords = async () => {
  const records = await db
    .select({
      ...
      borrowDate:
        sql<string>`to_char(${borrowRecords.borrowDate}, 'Mon DD YYYY')`.as(
          "borrowDate"
        ),
      dueDate: sql<string>`to_char(${borrowRecords.dueDate}, 'Mon DD YYYY')`.as(
        "dueDate"
      ),
      returnDate:
        sql<string>`to_char(${borrowRecords.returnDate}, 'Mon DD YYYY')`.as(
          "returnDate"
        ),
  ...
  try {
    ...
    const updatedBorrowRecord = await db
      .select({
        ...
        dueDate:
          sql<string>`to_char(${borrowRecords.dueDate}, 'Mon DD YYYY')`.as(
            "dueDate"
          ),
        returnDate:
          sql<string>`to_char(${borrowRecords.returnDate}, 'Mon DD YYYY')`.as(
...
export const getBorrowRequestStats = async () => {
  ...
  const totalBorrowRequest = await db
    .select({ count: count() })
    .from(borrowRecords)
